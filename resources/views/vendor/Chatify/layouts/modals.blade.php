  ...
  <div class="app-modal" data-name="delete">
      <div class="app-modal-container">
          <div class="app-modal-card" data-name="delete" data-modal='0'>
              <div class="app-modal-header">@lang('vendor/layouts.sure')</div>
              <div class="app-modal-body">@lang('vendor/layouts.undo')</div>
              <div class="app-modal-footer">
                  <a href="javascript:void(0)" class="app-btn cancel">@lang('vendor/layouts.cancel')</a>
                  <a href="javascript:void(0)" class="app-btn a-btn-danger delete">@lang('vendor/layouts.delete')</a>
              </div>
          </div>
      </div>
  </div>
      ...
      <div class="app-modal-container">
          <div class="app-modal-card" data-name="settings" data-modal='0'>
              <form id="updateAvatar" action="{{ route('avatar.update') }}" enctype="multipart/form-data" method="POST">
                  @csrf
                  <div class="app-modal-header">@lang('vendor/layouts.update')</div>
                  <div class="app-modal-body">
                      ...
                      <label class="app-btn a-btn-primary update">
                          @lang('vendor/layouts.upload')
                          <input class="upload-avatar" accept="image/*" name="avatar" type="file" style="display: none" />
                      </label>
                      {{-- Dark/Light Mode  --}}
                      <p class="divider"></p>
                      <p class="app-modal-header">@lang('vendor/layouts.dark') <span class="
                        {{ Auth::user()->dark_mode > 0 ? 'fas' : 'far' }} fa-moon dark-mode-switch"
                         data-mode="{{ Auth::user()->dark_mode > 0 ? 1 : 0 }}"></span></p>
                      {{-- change messenger color  --}}
                      <p class="divider"></p>
                      <p class="app-modal-header">@lang('vendor/layouts.change',['app_name' => config('app.name')])</p>
                  ...
                  <div class="app-modal-footer">
                      <a href="javascript:void(0)" class="app-btn cancel">@lang('vendor/layouts.cancel')</a>
                      <input type="submit" class="app-btn a-btn-success update" value="@lang('vendor/layouts.update_btn')" />
                  </div>
              </form>
          </div>
      </div>
